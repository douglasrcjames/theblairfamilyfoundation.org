...
export default class Partners extends Component {
    ...
    componentDidMount(){
        const allPartners = shuffle(partners);
        const half = Math.ceil(allPartners.length / 2);    

        const firstHalf = allPartners.splice(0, half)
        const secondHalf = allPartners.splice(-half)
    ...
    render() {
        if(!this.state.firstHalf || !this.state.secondHalf){
            return(
            ...
            return (
                <div>
                    <Slider {...settings1}>
                        {this.state.firstHalf.map((partner, index) => (
                            <div key={index}>
                                <a 
                                    href={partner.webUrl}
                                    target="_blank"
                    ...
                    <Slider {...settings2}>
                        {this.state.secondHalf.map((partner, index) => (
                            <div key={index}>
                                <a 
                                    href={partner.webUrl}
                                    target="_blank"
